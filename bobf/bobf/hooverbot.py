...
# Hooverbot is designed to be utility for main bot, that hoovers any low discount items from board, to regenerate more with perhaps better discounts.
# It has a bit of intelligence to drink potions when needed, and head to exit when done, so it can take care of itself
...
def sort_by_discount(items):
    items.sort(key=lambda p: (p["discountPercent"]))
    return items
...
def decide_destination(gamestate):
    ...
    items = [item for item in gamestate["items"] if item["price"]*(100-item["discountPercent"])/100 < my_bot["money"]]

    # Only pickup low discount items from board
    items = [item for item in items if item["discountPercent"] < 65]
